...
const Header = ({ buttonPrevIcon, buttonNextIcon }: HeaderProps) => {
  ...
  const renderPrevButton = (
    <Pressable
      ...
      onPress={() => {
        if (calendarView === CalendarViews.day) onChangeMonth(-1)
        else if (calendarView === CalendarViews.month) onChangeYear(currentYear - 1)
        else if (calendarView === CalendarViews.year) onChangeYear(currentYear - YEAR_PAGE_SIZE)
      }}
      testID="btn-prev"
      accessibilityRole="button"
      accessibilityLabel="Prev">
      <View style={[styles.iconContainer, styles.prev, theme?.headerButtonStyle]}>
        {buttonPrevIcon ?? (
          <Image
      ...
      onPress={() =>
        calendarView === CalendarViews.day
          ...
          : calendarView === CalendarViews.month
            ...
            : calendarView === CalendarViews.year && onChangeYear(currentYear + YEAR_PAGE_SIZE)
      }
      testID="btn-next"
      accessibilityRole="button"
      accessibilityLabel="Next">
      <View style={[styles.iconContainer, styles.next, theme?.headerButtonStyle]}>
        {buttonNextIcon ?? (
          <Image
    ...
    return (
      <>
        <Pressable
          onPress={() => {
            setCalendarView(
              calendarView === CalendarViews.year ? CalendarViews.day : CalendarViews.year,
            )
          ...
          <View style={[styles.textContainer, theme?.headerTextContainerStyle]}>
            <Text maxFontSizeMultiplier={1.4} style={[styles.text, theme?.headerTextStyle]}>
  ...
  const monthSelector = (
    <>
      <Pressable
        onPress={() =>
          setCalendarView(
            calendarView === CalendarViews.month ? CalendarViews.day : CalendarViews.month,
          )
        }
        testID="btn-month"
        accessibilityRole="button"
  ...
  return (
    <View style={[styles.headerContainer, theme?.headerContainerStyle]} accessibilityRole="header">
